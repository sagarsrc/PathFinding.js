...
Benchmark A* pathfinding on two maps and capture metrics.
...
1. Explore the pathfinding library in `src/` — find A*
2. Create a benchmark script that runs A* on BOTH maps
3. Capture per map: nodes explored, path length, execution time (avg over 1000 runs)
4. Write tests first (TDD)

## Output

Save results to `workers/racer-astar/output/results.md` (relative to fleet root).
